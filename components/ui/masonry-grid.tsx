...
interface MasonryProps {
  ...
  onItemClick?: (item: Item) => void; // 👈 callback click
}
...
const Masonry: React.FC<MasonryProps> = ({
  ...
  const { grid, containerHeight } = useMemo(() => {
    ...
    return { grid: mapped, containerHeight: Math.max(...colHeights) };
  }, [columns, items, width]);
    ...
    >
      {grid.map(item => (
        <div
          ...
          onClick={() => onItemClick?.(item)} // 👈 dùng callback thay vì window.open
