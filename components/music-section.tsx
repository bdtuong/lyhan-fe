...
export function MusicSection() {
  return (
    <section id="next-section" className="min-h-screen flex flex-col md:flex-row items-center justify-center gap-12 overflow-hidden py-12">
      {/* Ảnh với Atropos + ambient glow */}
      <motion.div
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center z-0">
          <div className="w-[500px] h-[500px] rounded-full bg-gradient-radial from-blue-500/30 to-transparent blur-3xl" />
        </div>

        {/* Atropos Image */}
        <Atropos className="relative z-10 w-full max-w-md bg-transparent" shadow={false} highlight={false}>
          <img
            src="/lh-1.png"
            alt="Music"
      ...
      >
        ...
        <Link href="/music" className="w-full md:w-4/5">
          <LiquidGlass
            className="p-8 text-left cursor-pointer flex flex-col items-start"
            cornerRadius={24}
            blurAmount={0.15}
          ...
          >
            <div className="flex items-center gap-4 mb-6">
              ...
              <h3 className="text-3xl font-semibold group-hover:text-primary transition-colors">
          ...
          {songs.map((song) => (
            <Link key={song.title} href={song.href}>
              ...
              >
                ...
                {/* Play icon ở cuối thẻ */}
                <PlayCircle
                  className="w-8 h-8 text-white/70 opacity-70 group-hover:text-white group-hover:opacity-100 transition-colors duration-300"
                />
              </LiquidGlass>
            </Link>
          ))}
